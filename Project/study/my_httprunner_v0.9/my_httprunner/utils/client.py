...
class Client:
    ...
    def send_http_v1(self, request_data):
        request_data = DataHandler.handle_template(request_data, DataHandler.cache)
        print(f'替换后的请求参数:{request_data}')
        resp = requests.request(**request_data)
        # pprint(resp.json())  # 快速引包，alt+回车
        print(resp.json())
        # return resp
        return resp  # resp对象，包含响应结果和请求内容，整个会话的内容都在resp里面
...
if __name__ == '__main__':
    ...
    Client = Client()
    Client.run_datas(requests_data)
