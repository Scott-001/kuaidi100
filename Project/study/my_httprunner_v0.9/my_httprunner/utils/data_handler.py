...
class DataHandler:
    ...
    @classmethod
    def _cache_data(cls, obj, path, cache_obj_key, cache):
        '''
        :param obj: 响应结果
    ...
    @classmethod
    def cache_data(cls, resp, extract):
        # resp_json = resp.json()
        for key, value in extract.items():
            # key = k2
            # value = json.args.k2
            # value = 真实的提取后的参数
            obj, path = value.split('.', 1)
            resp_json = eval(f'resp.{obj}()')  # 这行什么意思，相当于obj=resp.json()

            value = jmespath.search(path, resp_json)  # 提取参数
            cache_data = {key, value}  # 构建缓存的参数
            # cls.cache.update(cache_data)  # 缓存参数
            # 必须是字典对象才能用update
            cls.cache[key] = value  # 缓存参数
            # dict.update 字典存在则更新，不存在添加
